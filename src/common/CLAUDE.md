...
- **`frontend/`**: `bootstrap/` (mounts the React root), `design-language/` (design tokens, global styles, `Card`/`Button`), `layout/` (`TopNavBar`), `api/` (`http-client.ts`, the `/api`-prefixed fetch wrapper).
