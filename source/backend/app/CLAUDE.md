...
Feature code, one directory per domain — `auth`, `tasks`, `projects`, `blocks`, `recurrence`,
`journals`, `settings`, `workspaces`. Each domain owns its object module(s) (e.g. `tasks/task.py`),
its Flask blueprint (`*_routes.py`), and a colocated `tests/` directory. Each has its own short
CLAUDE.md.
